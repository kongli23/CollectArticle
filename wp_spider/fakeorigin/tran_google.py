...
def google_start(text):
    ...
    '''
    gg = Google()
    rep_str = re.compile('<div>*.*</div>')
    str_html = rep_str.sub('#####',text)
    result = gg.translate('zh-CN', 'en', str_html)
...
if __name__ == '__main__':
    ...
    gg = Google()
    # text = '你好， 新的我'
    with open('tag_cn.txt','r',encoding='utf-8') as f:
        text = f.read().strip()
        rep_str = re.compile('<div>*.*</div>')
        text = rep_str.sub('#####',text)
